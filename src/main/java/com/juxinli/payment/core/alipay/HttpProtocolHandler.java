...
public class HttpProtocolHandler {
	 ...
	 *             , IOException
	 */
	public PaymentHttpResponse execute( PaymentHttpRequest request, String strParaFileName,
		...
		// get模式且不带上传文件
		if ( request.getMethod().equals( PaymentHttpRequest.METHOD_GET ) ) {
			...
			for ( int i = 0; i < request.getParameters().length; i++ ) {
				parts.add( new StringPart(
						request.getParameters()[i].getName(), request
		...
		try {
			...
			if ( request.getResultType().equals( PaymentHttpReusltType.STRING ) ) {
				response.setStringResult( method.getResponseBodyAsString() );
			} else if ( request.getResultType().equals( PaymentHttpReusltType.BYTES ) ) {
